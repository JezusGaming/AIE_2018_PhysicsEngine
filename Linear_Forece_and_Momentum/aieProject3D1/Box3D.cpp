...
Box3D::Box3D(glm::vec3 position, glm::vec3 velocity, float mass, float width, float hight, float depth, glm::vec4 colour) : Rigidbody3D(ShapeType::BOX, position, velocity, 0, mass, 0.2f, 0.2f, 0.8f)
{
	m_width = width;
	m_hight = hight;
	m_depth = depth;
